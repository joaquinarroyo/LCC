...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            npx, npy = next_position
            if not self.walls[npx][npy]:
                next_corners_visited = tuple(
                    c for c in self.corners if c == next_position or c in corners_visited)
                successors.append(
                    ((next_position, next_corners_visited), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    if set(corners_visited) == set(corners):
        return 0

    h = 0
    # Filter out the corners that have been visited
    corners_l = filter(lambda x: x not in corners_visited, list(corners))

    while corners_l:
        # Find the closest corner and add its distance to the heuristic value
        distances = {}
        for corner in corners_l:
            d = util.manhattanDistance(position, corner)
            distances[d] = corner

        distances_k = distances.keys()
        min_d = min(distances_k)
        min_corner = distances[min_d]

        # Remove the corner from the corners list and update the position to the closest corner
        corners_l.remove(min_corner)
        position = min_corner
