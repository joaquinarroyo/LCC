...
static const unsigned FILE_SIZE = CONTENT_SIZE * 10;

static void
FileWrite(unsigned sector = 1)
{
    printf("Sequential write of %u byte file, in %u byte chunks in sector %u\n",
           FILE_SIZE, CONTENT_SIZE, sector);

    if (!fileSystem->Create(FILE_NAME, 0, sector)) {
...
CreateDir()
{
    ...
    if (!fileSystem->Mkdir(DIR_NAME)) {
        fprintf(stderr, "Perf test: unable to create directory %s\n", DIR_NAME);
        return;
    }
...
ChangeDirectory()
{
    ...
    unsigned sector = fileSystem->Cd(DIR_NAME);
    if (!sector) {
        fprintf(stderr, "Perf test: unable to change to directory %s\n", DIR_NAME);
        return -1;
    }
    printf("Changed to directory %s successfully (sector %u)\n", DIR_NAME, sector);
    return sector;
}
...
PerformanceTest()
{
    ...
    fileSystem->Ls();
    unsigned sector = ChangeDirectory();
    FileWrite(sector);  // This creates another file in the sub-directory.
