...
/// * `name` is the file name to look up.
int
Directory::Find(const char *name, bool findDir)
{
    ASSERT(name != nullptr);

    int i = FindIndex(name);
    if (i != -1) {
        if (findDir && !raw.table[i].isDir) return -1;
    ...
    for (unsigned i = 0; i < raw.tableSize; i++) {
        if (raw.table[i].inUse) {
            if (raw.table[i].isDir) {
                printf("\033[34m%s\033[0m ", raw.table[i].name);
            } else {
                printf("\033[32m%s\033[0m ", raw.table[i].name);
            }
        }
    }
    printf("\n");
